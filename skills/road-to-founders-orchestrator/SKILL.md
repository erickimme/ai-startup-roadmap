...
- **full (2일 기본·깊이 우선)**: 단계당 ~30분. 질문 최대 5개, 단계마다 깊게 논의하고 `korean-synthetic-consumer`·`evidence-research`를 보조로 활용해 **구체적인** 산출물을 만든다.
- **fast (시간 압박·1일)**: 단계당 ~20분. 질문은 **최대 3개**만, 답이 없으면 `[가정]`으로 채워 v0를 먼저 완성한다. 합성 소비자·근거 리서치·심화 레드팀은 **건너뛰고** 빠른 참가자만 추가한다.
...
각 단계를 끝낼 때 **DOCX로 내보내** 참가자가 Word에서 수정하게 한다: `bash scripts/export-docx.sh` → `roadmap/docx/NN_*.docx`(단계별) + 통합본. 참가자가 Word에서 고치면 그 내용을 말로 받아 md를 갱신하거나 `bash scripts/sync-docx.sh`로 재반영한 뒤 다음 단계로 진행한다. (md는 작업 원본, docx는 사람이 보고 고치는 사본)
