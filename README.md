...
├── skills/          # 단계별 스킬 16종 + manifest.json (orchestrator가 라우팅) — Day 2 빌드·디자인(vibe-build·design-polish) 포함
...
Day 1에 만든 로드맵을 **실제 동작하는 제품**으로 만들고 데모합니다.

**1) 어제 이어서 (제일 먼저)** — 어제 clone한 `ai-startup-roadmap` 폴더를 열고, 새 스킬을 받기 위해 최신화합니다.
```bash
git pull        # 어제 이후 추가된 Build·디자인 스킬 받기 (roadmap/ 산출물은 보존)
codex           # 다시 실행 → "roadmap/ 어제 산출물 이어서 다음 단계부터"
```

**2) 배포 (`vibe-build`)** — 트랙을 고릅니다.
- 🅰 **Landing**(기본·전원): 가치 + 문제/해결 + **waitlist 폼(필수)** → 배포(URL)
- 🅱 **MVP**(야심): 핵심 기능 1개 흐름
- 스택: **GitHub + Vercel + Supabase**(Next.js). 사람은 로그인(auth)만, 나머지는 Codex가 CLI로. waitlist는 Supabase에 저장(RLS: 익명 INSERT만 허용).

**3) 디자인 (`design-polish`)** — 배포 후 **레퍼런스 기반**으로 "AI 티" 없이 끌어올립니다. (Mobbin·실제 사이트 → 디자인 토큰 → shadcn/ui) 자세히는 [`docs/디자인-가이드.md`](docs/디자인-가이드.md).

> 한 제품 = **한 Vercel 프로젝트 = 한 URL**을 권장합니다(랜딩 `/`, MVP `/app`). 재배포는 같은 URL을 갱신합니다.
