...
`roadmap/`에 쌓인 9종 문서를 **하나의 Word(.docx)** 로 내보냅니다.

```bash
bash scripts/export-docx.sh        # → roadmap/나의_창업로드맵.docx
```

- 코치에게 **"문서로 내보내줘"** 라고 말하면 Codex가 위 스크립트를 실행해 줍니다(승인 필요).
- 표까지 깔끔히 변환하려면 **pandoc**이 필요합니다: macOS `brew install pandoc` / Windows `choco install pandoc` / Linux `sudo apt install pandoc`. (자세히는 `docs/troubleshooting.md`)
- 실제 변환 예시는 `examples/sample-run-건강정보플랫폼/` 의 9종 문서로 확인할 수 있습니다.
